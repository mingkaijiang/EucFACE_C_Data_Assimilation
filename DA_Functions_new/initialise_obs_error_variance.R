initialise_obs_error_variance <- function(s, err_var_obs, obs) {
        ...
        err_var_obs[s$POS_RA,] = ifelse(!is.na(obs[,s$POS_RA]), 0.05, NA)   # %
        ...
        err_var_obs[s$POS_CR,] = ifelse(!is.na(obs[,s$POS_CR]), 0.1, NA)   # %
        ...
        err_var_obs[s$POS_CL,] = ifelse(!is.na(obs[,s$POS_CL]), 0.1, NA)    # %
        err_var_obs[s$POS_CS,] = ifelse(!is.na(obs[,s$POS_CS]), 0.1, NA)    # %
        err_var_obs[s$POS_GPP,] = ifelse(!is.na(obs[,s$POS_GPP]), 0.1, NA)   # %
